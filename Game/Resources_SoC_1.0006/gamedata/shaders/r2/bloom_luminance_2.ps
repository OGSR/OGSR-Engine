...
float	sample	(float2	tc)	{
	...
	// emulate bilinear
float	sample	(float2	tc)	{
	float 	pfloat 	= .5h/64.h;
	float4 	res;
		res.x 	= dot	(tex2D(s_image,tc+float2(-pfloat,-pfloat)), 1/4.h); // sum components
		res.y 	= dot	(tex2D(s_image,tc+float2(+pfloat,-pfloat)), 1/4.h); // sum components
		res.z 	= dot	(tex2D(s_image,tc+float2(-pfloat,+pfloat)), 1/4.h); // sum components
		res.w 	= dot	(tex2D(s_image,tc+float2(+pfloat,+pfloat)), 1/4.h); // sum components
