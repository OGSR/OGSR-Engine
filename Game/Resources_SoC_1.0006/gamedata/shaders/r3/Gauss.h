...
float4 Gauss(Texture2D t2d, float2 texCoord, int factor, bool optimize)
{
	...
	if(!optimize)
	{
		...
		outColor.rgb =   
		(		
			1.f * t2d.SampleLevel(smp_rtlinear, float3(texCoord,0), 0).rgb +
			1.f * t2d.SampleLevel(smp_rtlinear, float3(texCoord + float2(dx, 0),0), 0).rgb +
			1.f * t2d.SampleLevel(smp_rtlinear, float3(texCoord + float2(dy, 0),0), 0).rgb +
			1.f * t2d.SampleLevel(smp_rtlinear, float3(texCoord + float2(-dx, 0),0), 0).rgb +
