...
void CHudItem::Load(LPCSTR section)
{
	...
	m_jump_offset[2][0].set(READ_IF_EXISTS(pSettings, r_bool, section, "jump_enabled", true), READ_IF_EXISTS(pSettings, r_float, section, "jump_transition_time", 0.45f), READ_IF_EXISTS(pSettings, r_float, section, "fall_transition_time", 0.7f)); // normal
	m_jump_offset[2][1].set(READ_IF_EXISTS(pSettings, r_bool, section, "jump_aim_enabled", true), READ_IF_EXISTS(pSettings, r_float, section, "jump_aim_transition_time", 0.45f), READ_IF_EXISTS(pSettings, r_float, section, "fall_aim_transition_time", 0.7f)); // aim-GL
	 ////////////////////////////////////////////
...
u32 CHudItem::PlayHUDMotion(const char* M, const bool bMixIn, const u32 state, const bool randomAnim)
{
	...
	if (Det && Det->m_parent_hud_item != this && Wpn && Wpn->m_parent_hud_item == this && (smart_cast<CWeapon*>(this) || smart_cast<CMissile*>(this)) && Det->m_parent_hud_item->GetState() == eIdle) {
...
// TODO: Придумать более годный способ получения полиции и направления для коллизии болтов, детекторов, биноклей и ножей. У стволов есть shoot_point - там легко считать от него, в отличие от.
// TODO: Придумать какой-то способ получения нужной позиции для рейтрейса. Сейчас сделал не оптиматьно, по сути худ считается два раза за фрейм (не полностью, но такое решение всё равно выглядит плохо)
...
void CHudItem::UpdateCollision(Fmatrix& trans) {
	...
	Level().ObjectSpace.RayQuery(RQR, RD, [](collide::rq_result& result, LPVOID params)
		{
	...
	const Fvector pos_steps{ fabsf(m_nearwall_target_hud_offset.x / DMGR_STEP_COEF) * speed_coef , fabsf(m_nearwall_target_hud_offset.y / DMGR_STEP_COEF) * speed_coef , fabsf(m_nearwall_target_hud_offset.z / DMGR_STEP_COEF) * speed_coef };
	const Fvector rot_steps{ fabsf(m_nearwall_target_hud_rotate.x / DMGR_STEP_COEF) * speed_coef ,fabsf(m_nearwall_target_hud_rotate.y / DMGR_STEP_COEF) * speed_coef , fabsf(m_nearwall_target_hud_rotate.z / DMGR_STEP_COEF) * speed_coef };
