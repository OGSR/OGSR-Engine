...
{
	//попытка стрел€ть когда нет патронов
	if(GetState() == eIdle) 
	{
		OnEmptyClick			();
		return;
...
void CWeaponMagazined::ReloadMagazine() 
{
	...
	//переменна€ блокирует использование
...
void CWeaponMagazined::UpdateCL			()
{
	...
	//когда происходит апдейт состо€ни€ оружи€
...
bool CWeaponMagazined::Attach(PIItem pIItem, bool b_send_event)
{
	...
	if(result)
	{
		...
		{
			//уничтожить подсоединенную вещь из инвентар€
//.			pIItem->Drop					();
...
void CWeaponMagazined::InitAddons()
{
	...
	else
	{
		...
		if (IsZoomEnabled())
		{
			//KRodin: «ачем??? Ќа стволах без прицела если нужно приближение - надо править ironsight_zoom_factor, а не городить тут такие костыли.
			//¬прочем, дл€ ќ√—≈ оно нужно, обнесу этим дефайном, чтоб не городить новый.
...
//переключение режимов стрельбы одиночными и очеред€ми
