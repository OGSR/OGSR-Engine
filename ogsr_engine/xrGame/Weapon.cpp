...
void CWeapon::UpdateCL()
{
    ...
    if (AllowBore())
    {
        if (GetNextState() == GetState() && H_Parent() == Level().CurrentEntity())
        {
            ...
            {
                if (g_bHudAdjustMode == 0 && GetState() == eIdle && (Device.dwTimeGlobal - m_dw_curr_substate_time > 20000) && !IsZoomed() &&
...
constexpr const char* wpn_scope = "wpn_scope";
constexpr const char* wpn_silencer = "wpn_silencer";
constexpr const char* wpn_grenade_launcher = "wpn_launcher";
...
float CWeapon::CurrentZoomFactor() { return IsScopeAttached() ? m_zoom_params.m_fScopeZoomFactor : m_zoom_params.m_fIronSightZoomFactor; };
...
void CWeapon::OnZoomIn()
{
    ...
    if (m_zoom_params.m_sUseZoomPostprocess.size() && IsScopeAttached())
    {
        ...
        if (actor && m_zoom_params.m_pNight_vision == nullptr)
...
void CWeapon::SwitchState(u32 S)
{
    ...
    if (bDebug)
    {
        Msg("---Server is going to send GE_WPN_STATE_CHANGE to [%d], weapon_section[%s], parent[%s]", S, cNameSect().c_str(),
...
bool CWeapon::MovingAnimAllowedNow() const { return inherited::MovingAnimAllowedNow() && !IsZoomed(); }
...
#pragma todo("SIMP: хз, надо ли здесь")
    /*
    VisMask new_mask;
    new_mask.set_all();
...
float CWeapon::GetHudFov()
{
    ...
    //SIMP: под 3DSS в новом виде надо это всё переосмыслить, сделать 3dss как у деда. Пока в этом плане ничего не делал. Пока и тестить не на чем так то
