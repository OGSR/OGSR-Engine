...
static void tune_remap(const Ivector& in_values, Ivector& out_values) {
	...
	} else if (pInput->iGetAsyncKeyState(DIK_Z)) { //strict by X
		out_values.x = in_values.y;
		out_values.y = 0;
		out_values.z = 0;
	} else if (pInput->iGetAsyncKeyState(DIK_X)) { //strict by Y
		out_values.x = 0;
		out_values.y = in_values.y;
		out_values.z = 0;
	} else if (pInput->iGetAsyncKeyState(DIK_C)) { //strict by Z
		out_values.x = 0;
		out_values.y = 0;
...
void player_hud::tune(const Ivector& _values) {
	...
	if (g_bHudAdjustMode == HUD_POS || g_bHudAdjustMode == HUD_ROT)
	{
		...
		if ((values.x) || (values.y) || (values.z)) {
			...
			if (auto hi = m_attached_items[g_bHudAdjustItemIdx])
				hi->tune(values);
}
...
void hud_draw_adjust_mode() {
	...
	const char* _text{};
	if (pInput->iGetAsyncKeyState(DIK_LSHIFT))
		_text = "press SHIFT+NUM 0-return|1-hud_pos|2-hud_rot|3-itm_pos|4-itm_rot|5-fire_point|6-fire_point2|7-shell_point|8-pos_step|9-rot_step    ||||||    press SHIFT+1-laser_point|2-flashlight_point";
	...
	if (_text)
	{
		...
		F->OutNext(_text);
		F->OutNext("for item: [%d] [%s]", g_bHudAdjustItemIdx, g_player_hud->attached_item(u16(g_bHudAdjustItemIdx)) ? g_player_hud->attached_item(u16(g_bHudAdjustItemIdx))->m_sect_name.c_str() : "NOT FOUND");
