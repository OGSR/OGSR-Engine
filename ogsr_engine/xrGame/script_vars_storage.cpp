...
void SCRIPT_VAR::release()
{
	...
	{		
		sv.T->release(); // могут остатьс€ ссылки в скриптах
		return;
	}
...
void* SCRIPT_VAR::smart_alloc(int new_type, u32 cb) // схема автоматического выделени€ пам€ти
{
	...
	if (type & SVT_ALLOCATED) // если стара€ переменна€ была аллоцирована в пам€ти
	{
	...
	{		
		...
		{  // default var type
			if (sv.size > sizeof(sv.s_value))
			{
				...
				if (LUA_TSTRING == sv.eff_type() && -1 == *(int*)sv.data) // €вный выход за пределы чтени€
...
/*
		...
		{			
			...
			MsgCB(msg); // WARN: имплементаци€ этой функции должна быть из последней ревизии с 4-кебибайтными буферами
...
void CScriptVarsTable::set(lua_State *L, LPCSTR k, int index, int key_type)
{
	...
	case LUA_TUSERDATA:
	{
		...
		{
			sv.smart_alloc(LUA_TNETPACKET); // избежание утечек пам€ти
		...
		void *dst = sv.smart_alloc(new_type, sv.size); // избежание утечек пам€ти		
...
int lua_pushsvt(lua_State *L, CScriptVarsTable *T)
{
	...
	sv->size = 4;
	sv->T = T;	
	T->add_ref();  // регистраци€ выдачи экземпл€ра в lua
...
{
	CScriptVarsTable *svt = xr_new<CScriptVarsTable>(); // здесь добавл€ть ссылку не надо, т.к. объект нигде не сохран€етс€!
	if (lua_isstring(L, 1))
...
int script_vars_import(lua_State *L) // загрузка таблицы переменных из нет-пакета 
{
	...
	if (lua_isuserdata(L, 1))
		svt = lua_tosvt(L, 1);
	if (!svt)
		svt = xr_new<CScriptVarsTable>(); // здесь добавл€ть ссылку не надо, т.к. объект нигде не сохран€етс€!
