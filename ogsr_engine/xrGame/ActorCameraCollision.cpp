...
static void cammera_shell_collide_callback_common(bool& do_collide, bool bo1, dContact& c, SGameMtl* material_1, SGameMtl* material_2)
{
    if (!do_collide)
        return;
    do_collide = false;
    const SGameMtl* oposite_matrial = bo1 ? material_1 : material_2;
    if (oposite_matrial->Flags.test(SGameMtl::flPassable))
        return;

    const dxGeomUserData* my_data = retrieveGeomUserData(bo1 ? c.geom.g1 : c.geom.g2);
    const dxGeomUserData* oposite_data = retrieveGeomUserData(bo1 ? c.geom.g2 : c.geom.g1);
    ...
    const dJointID contact_joint = dJointCreateContactSpecial(nullptr, ContactGroup, &c);
...
static void cammera_shell_collide_callback(bool& do_collide, bool bo1, dContact& c, SGameMtl* material_1, SGameMtl* material_2)
{
    ...
    if (oposite_data && oposite_data->ph_ref_object && oposite_data->ph_ref_object->IsStalker())
        return;
...
static void get_viewport_geom(Fvector& box, Fmatrix& form, const CCameraBase& camera, float _viewport_near)
{
    ...
    tviewport_size(Device, _viewport_near, camera, box.x, box.y);
    form.identity();
...
const u16 cam_correction_steps_num = 10;
...
void get_old_camera_box(Fvector& old_box_size, Fmatrix& old_form, CPhysicsElement* roote, CBoxGeom* box)
{
...
void set_camera_collision(const Fvector& box_size, const Fmatrix& xform, CPhysicsElement* roote, CBoxGeom* box)
{
    ...
    const Fvector character_collision_box_size =
        Fvector().add(box_size, Fvector().set(camera_collision_character_skin_depth, camera_collision_character_skin_depth, camera_collision_character_skin_depth));
...
bool test_camera_box(const Fvector& box_size, const Fmatrix& xform, CPhysicsShellHolder* l_actor)
{
    ...
    const bool ret = do_collide_not_move(xform, l_actor, shell, roote);
