...
public:
    ...
    bool ZoomHideCrosshair() { return (m_zoom_params.m_bHideCrosshairInZoom || ZoomTexture()) && !psActorFlags.test(AF_CROSSHAIR_DBG); }
...
protected:
    ...
    virtual bool MovingAnimAllowedNow() const;
...
protected:
    size_t GetWeaponTypeForCollision() const override { return CWeaponBase; }
    Fvector GetPositionForCollision() override { return get_LastShootPoint(); }
    Fvector GetDirectionForCollision() override { return get_LastFD(); }
...
public:
    ...
    bool ParentIsActor() const override;
