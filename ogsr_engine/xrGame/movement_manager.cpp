...
void CMovementManager::Load(LPCSTR section) { locations().Load(section); }
...
void CMovementManager::on_frame(CPHMovementControl* movement_control, Fvector& dest_position)
{
    ...
    g_pGamePersistent->GrassBendersUpdate(object().ID(), grassbender_id, grassbender_frame, object().Position(), -1.0f, 1.0f);
}
