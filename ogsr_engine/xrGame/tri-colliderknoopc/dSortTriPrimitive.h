...
template <class _T>
IC int dcTriListCollider::dSortTriPrimitiveCollide(_T primitive, dxGeom* o1, dxGeom* o2, int flags, dContactGeom* contact, int skip, const Fvector& AABB)
{
            ...
            if ((!(last_pos_dist < 0.f)) || b_pushing || isnan(last_pos_dist))
                if (__aabb_tri(Point(p), Point((float*)&AABB), vertices))
                {
                    ...
                    if (contain_pos)
                    {
                        ...
                        if (neg_depth > tri.depth && (!(*pushing_neg || spushing_neg) || dDOT(neg_tri.norm, tri.norm) > -M_SQRT1_2) &&
                            (!(*pushing_b_neg || spushing_b_neg) ||
                             dDOT(b_neg_tri.norm, tri.norm) > -M_SQRT1_2)) // exclude switching on opposite side &&(!*pushing_b_neg||dDOT(b_neg_tri->norm,tri.norm)>-M_SQRT1_2)
                        {
                            neg_depth = tri.depth;
                            neg_tri = tri;
                            data->neg_tri = tri.T;
