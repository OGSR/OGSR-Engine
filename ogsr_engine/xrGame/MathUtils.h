...
IC void dVectorDeviation(const dReal* vector3_from, const dReal* vector3_to, dReal* vector_dev) { dVectorSub(vector_dev, vector3_to, vector3_from); }
...
IC bool check_obb_sise(Fobb& obb) { return (!fis_zero(obb.m_halfsize.x, EPS_L) || !fis_zero(obb.m_halfsize.y, EPS_L) || !fis_zero(obb.m_halfsize.z, EPS_L)); }
...
IC float ThrowMinVelTime(const Fvector& transference, float gravity_accel) { return _sqrt(2.f * transference.magnitude() / gravity_accel); }
