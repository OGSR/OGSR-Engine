...
IC bool CPHCallOnStepCondition::time_out() const { return ph_world->m_steps_num > m_step; }

bool CPHCallOnStepCondition::is_true() { return time_out(); }
bool CPHCallOnStepCondition::obsolete() const { return time_out(); }

void CPHCallOnStepCondition::set_steps_interval(u64 steps) { set_step(ph_world->m_steps_num + steps); }
...
CPHConstForceAction::CPHConstForceAction(CPhysicsShell* shell, const Fvector& force) : CPHShellBasedAction(shell) { m_force.set(force); }
