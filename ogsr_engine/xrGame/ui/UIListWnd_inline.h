...
bool CUIListWnd::AddItem(const char* str, const float shift, void* pData, int value, int insertBeforeIdx)
{
    ...
    pItem->Init(str, shift, m_bVertFlip ? GetHeight() - GetItemsCount() * m_iItemHeight - m_iItemHeight : GetItemsCount() * m_iItemHeight, m_iItemWidth, m_iItemHeight);
...
bool CUIListWnd::AddItem(Element* pItem, int insertBeforeIdx)
{
    ...
    pItem->Init(pItem->GetWndRect().left, m_bVertFlip ? GetHeight() - GetItemsCount() * m_iItemHeight - m_iItemHeight : GetItemsCount() * m_iItemHeight, m_iItemWidth,
                m_iItemHeight);
