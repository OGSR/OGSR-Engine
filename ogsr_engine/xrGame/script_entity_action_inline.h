...
IC void CScriptEntityAction::SetAction(CScriptMovementAction& tMovementAction) { SetAction(tMovementAction, m_tMovementAction); }

IC void CScriptEntityAction::SetAction(CScriptWatchAction& tWatchAction) { SetAction(tWatchAction, m_tWatchAction); }

IC void CScriptEntityAction::SetAction(CScriptAnimationAction& tAnimationAction) { SetAction(tAnimationAction, m_tAnimationAction); }

IC void CScriptEntityAction::SetAction(CScriptSoundAction& tSoundAction) { SetAction(tSoundAction, m_tSoundAction); }

IC void CScriptEntityAction::SetAction(CScriptParticleAction& tParticleAction) { SetAction(tParticleAction, m_tParticleAction); }

IC void CScriptEntityAction::SetAction(CScriptObjectAction& tObjectAction) { SetAction(tObjectAction, m_tObjectAction); }

IC void CScriptEntityAction::SetAction(CScriptActionCondition& tActionCondition) { SetAction(tActionCondition, m_tActionCondition); }

IC void CScriptEntityAction::SetAction(CScriptMonsterAction& tMonsterAction) { SetAction(tMonsterAction, m_tMonsterAction); }

IC void CScriptEntityAction::SetAction(void* user_data) { m_user_data = user_data; }

IC bool CScriptEntityAction::CheckIfActionCompleted(const CScriptAbstractAction& tAbstractAction) const { return (tAbstractAction.m_bCompleted); }
...
IC bool CScriptEntityAction::CheckIfMonsterActionCompleted() const { return (CheckIfActionCompleted(m_tMonsterAction)); }
...
IC bool CScriptEntityAction::CheckIfActionCompleted()
{
    ...
    if (!m_tActionCondition.m_dwFlags && (m_tActionCondition.m_tLifeTime < 0) && CheckIfMovementCompleted() && CheckIfWatchCompleted() && CheckIfAnimationCompleted() &&
        CheckIfSoundCompleted() && CheckIfParticleCompleted() && CheckIfObjectCompleted() && CheckIfMonsterActionCompleted())
        return (true);
    else
        return (!l_dwFlags);
}
