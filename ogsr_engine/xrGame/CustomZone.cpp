...
void CCustomZone::Load(LPCSTR section)
{
    ...
    if (m_zone_flags.test(eIdleLight))
    {
        ...
        // SIMP: переименовал на чн/зп-манер. в огср настройка называется idle_light_volumetric_enable
        IdleLightVolumetric = READ_IF_EXISTS(pSettings, r_bool, section, "idle_light_volumetric", false);
...
void CCustomZone::feel_touch_new(CObject* O)
{
    ...
    if ((object_info.small_object && m_zone_flags.test(eIgnoreSmall)) || (object_info.nonalive_object && m_zone_flags.test(eIgnoreNonAlive)) ||
...
float CCustomZone::effective_radius(float nearest_shape_radius) { return /*Radius()*/ nearest_shape_radius * m_fEffectiveRadius; }

float CCustomZone::Power(float dist, float nearest_shape_radius) { return m_fMaxPower * RelativePower(dist, nearest_shape_radius); }
