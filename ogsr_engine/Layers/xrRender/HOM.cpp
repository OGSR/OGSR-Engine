...
{
	if (!bEnabled || ps_r2_ls_flags_ext.test(R2FLAGEXT_DISABLE_HOM)) return;
...
{
	if (!bEnabled || ps_r2_ls_flags_ext.test(R2FLAGEXT_DISABLE_HOM)) return TRUE;
...
BOOL CHOM::visible		(vis_data& vis)
{
	...
	if (!bEnabled || ps_r2_ls_flags_ext.test(R2FLAGEXT_DISABLE_HOM)) return TRUE; // return - everything visible
