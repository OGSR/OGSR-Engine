...
struct ttapi_ThreadParams
{
	...
	ttapi_ThreadParams(std::function<void()> InFunc)
...
u32 ttapi_Init()
{
	...
	R_ASSERT(hCleanupEnv != nullptr);
	SetThreadpoolCallbackCleanupGroup(&CallbackEnv, hCleanupEnv, nullptr);
	//
	u32 th_count = CPU::ID.threadCount; //ID->coresCount
	...
	if (pszTemp)
		if (sscanf_s(pszTemp + strlen(szSearchFor), "%u", &dwOverride))
			if ((dwOverride >= 1) && (dwOverride <= CPU::ID.threadCount))
				th_count = dwOverride;
	Msg("TTAPI number of threads: [%u]", th_count);
	//
	SetThreadpoolThreadMinimum(hPool, th_count);
	SetThreadpoolThreadMaximum(hPool, th_count);
	WorkerCount = th_count;
...
u32 ttapi_GetWorkersCount()
{
	return WorkerCount;
}

void ttapi_AddWorker(std::function<void()> lpWorkerFunc)
{
	auto pParams = new ttapi_ThreadParams(lpWorkerFunc);
	auto hWork = CreateThreadpoolWork(ttapi_worker_threadentry, pParams, &CallbackEnv);
...
{
	CloseThreadpoolCleanupGroupMembers(hCleanupEnv, FALSE, nullptr);
}

void NTAPI ttapi_worker_threadentry(PTP_CALLBACK_INSTANCE Instance, PVOID Context, PTP_WORK Work)
{
	auto pParams = (ttapi_ThreadParams*)Context;
	pParams->Func();
	delete pParams;
}
...
typedef struct TTAPI_WORKER_PARAMS {
	...
	std::function<void()>	lpWorkerFunc;
	DWORD					dwPadding[13];
...
u32 ttapi_Init()
{
	...
	// System Info
	ttapi_workers_count = CPU::ID.coresCount > 1 ? 2 : CPU::ID.coresCount;
	...
	Msg("TTAPI number of threads: [%u]", ttapi_workers_count);
	...
	DWORD dwThreadId = 0;
	DWORD_PTR dwAffinitiMask = CPU::ID.affinity_mask;
