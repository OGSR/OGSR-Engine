...
#ifdef  _EDITOR
/*KD: по€сн€ю, зачем тут это. ¬ студии есть перегруженные шаблонные функции, которые берут макс. размер
из рзмера массива dest, переданного первым аргументом. ¬ Ѕилдере такого нет, надо указывать макс размер.
’ак, конечно... “е функции, что таки в Ѕилдере 2010 есть, € закомментил.*/
...
{
	// check for: Signaling NaN, Quiet NaN, Negative infinity ( ЦINF), Positive infinity (+INF), Negative denormalized, Positive denormalized
