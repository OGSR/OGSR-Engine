...
static IReader* open_archive_chunk(void* ptr, u32 ID, const char* archiveName, size_t archiveSize, const trivial_encryptor::key_flag key)
{
    ...
    while (true)
    {
        ...
        if ((dwType & ~CFS_CompressMark) == ID)
        {
            ...
            if (dwType & CFS_CompressMark)
            {
                ...
                if (shouldDecrypt)
                    g_trivial_encryptor.decode(src_data, dwSize, src_data, key); // Try WW key first
...
{
    IReader* hdr = open_archive_chunk(hSrcFile, 1, path.c_str(), size, trivial_encryptor::key_flag::russian);
    R_ASSERT(hdr);
    RStringVec fv;
    while (!hdr->eof())
    {
