...
IC _intrusive_ptr::intrusive_ptr() { m_object = 0; }
...
TEMPLATE_SPECIALIZATION
IC bool _intrusive_ptr::operator!() const { return (!m_object); }
...
TEMPLATE_SPECIALIZATION
IC bool _intrusive_ptr::equal(const self_type& rhs) const { return (m_object == rhs.m_object); }
...
TEMPLATE_SPECIALIZATION
IC const typename _intrusive_ptr::object_type* _intrusive_ptr::get() const { return (m_object); }

TEMPLATE_SPECIALIZATION
IC bool operator==(_intrusive_ptr const& a, _intrusive_ptr const& b) { return (a.get() == b.get()); }
...
TEMPLATE_SPECIALIZATION
IC void swap(_intrusive_ptr& lhs, _intrusive_ptr& rhs) { lhs.swap(rhs); }
