...
void CRenderDevice::End		(void)
{
	...
	if (dwPrecacheFrame)
	{
		...
		if (0==dwPrecacheFrame)
		{
			...
			xrRender_apply_tf(); //KRodin: вот это надо делать именно здесь, и нигде больше! »наче не работает.
		}
	}
	...
	if (!Device.m_SecondViewport.IsSVPFrame() && !Device.m_SecondViewport.m_bCamReady) //--#SM+#-- +SecondVP+ Ќе выводим кадр из второго вьюпорта на экран (на практике у нас экранна€ картинка обновл€етс€ минимум в два раза реже) [don't flush image into display for SecondVP-frame]
