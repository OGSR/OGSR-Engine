...
// Это значение используется внутри рендера, а следующее - внутри
// xrGame.
#define R_VIEWPORT_NEAR 0.05f

// KRodin: VIEWPORT_NEAR обычно уменьшают для возможности близкого приближения прицелов к камере акторя до 0.05f
// Но, уменьшая значение, имейте ввиду, что появятся многочисленные баги с мелькающими тенями/текстурами вдали (особенно заметно при прицеливании).
// Чем меньше VIEWPORT_NEAR - тем больше эти баги заметны.
// Поэтому теперь, чтобы можно было настроить это без пересборки движка, сделал удобную настройку по команде r_viewport_near
