...
public:
	// alpet: дл€ сохран€емости конфигурации источников света
	...
	virtual void					set_flare			(bool)								= 0;
	virtual void					set_volumetric		(bool)								= 0;
	virtual void					set_actor_torch		(bool)								= 0;
	virtual void					set_lsf_params(float _speed, float _amount, float _jit) = 0;
...
public:
	...
	virtual void BeforeWorldRender() = 0; //--#SM+#-- ѕеред рендерингом мира
	virtual void AfterWorldRender() = 0; //--#SM+#-- ѕосле рендеринга мира (до UI)
