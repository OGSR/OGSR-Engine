...
PropItem* __stdcall CPropHelper::FindItem(PropItemVec& items, shared_str key, EPropType type)
{
    if (type!=PROP_UNDEF){
...
CaptionValue*  	__stdcall CPropHelper::CreateCaption	(PropItemVec& items, shared_str key, shared_str val)
{	return	   	(CaptionValue*)	AppendValue	(items,key,xr_new<CaptionValue>(val),PROP_CAPTION);					}
CanvasValue*   	__stdcall CPropHelper::CreateCanvas	(PropItemVec& items, shared_str key, shared_str val, int height)
{	return	   	(CanvasValue*)	AppendValue	(items,key,xr_new<CanvasValue>(val,height),PROP_CANVAS);  			}
ButtonValue*   	__stdcall CPropHelper::CreateButton	(PropItemVec& items, shared_str key, shared_str val, u32 flags)
{	return	   	(ButtonValue*)	AppendValue	(items,key,xr_new<ButtonValue>(val,flags),PROP_BUTTON);				}
S8Value* 		__stdcall CPropHelper::CreateS8		(PropItemVec& items, shared_str key, s8* val, s8 mn, s8 mx, s8 inc)
{	return	   	(S8Value*)		AppendValue	(items,key,xr_new<S8Value>(val,mn,mx,inc,0),PROP_NUMERIC);			}
S16Value* 		__stdcall CPropHelper::CreateS16		(PropItemVec& items, shared_str key, s16* val, s16 mn, s16 mx, s16 inc)
{	return	   	(S16Value*)		AppendValue	(items,key,xr_new<S16Value>(val,mn,mx,inc,0),PROP_NUMERIC); 		}
S32Value* 	 	__stdcall CPropHelper::CreateS32		(PropItemVec& items, shared_str key, s32* val, s32 mn, s32 mx, s32 inc)
{   return		(S32Value*)		AppendValue	(items,key,xr_new<S32Value>(val,mn,mx,inc,0),PROP_NUMERIC); 		}
U8Value* 		__stdcall CPropHelper::CreateU8		(PropItemVec& items, shared_str key, u8* val, u8 mn, u8 mx, u8 inc)
{   return		(U8Value*)		AppendValue	(items,key,xr_new<U8Value>(val,mn,mx,inc,0),PROP_NUMERIC);   		}
U16Value* 		__stdcall CPropHelper::CreateU16		(PropItemVec& items, shared_str key, u16* val, u16 mn, u16 mx, u16 inc)
{   return		(U16Value*)		AppendValue	(items,key,xr_new<U16Value>(val,mn,mx,inc,0),PROP_NUMERIC); 		}
U32Value* 	  	__stdcall CPropHelper::CreateU32		(PropItemVec& items, shared_str key, u32* val, u32 mn, u32 mx, u32 inc)
{	return		(U32Value*)		AppendValue	(items,key,xr_new<U32Value>(val,mn,mx,inc,0),PROP_NUMERIC);			}
FloatValue*    __stdcall 	CPropHelper::CreateFloat	(PropItemVec& items, shared_str key, float* val, float mn, float mx, float inc, int decim)
{   return		(FloatValue*)	AppendValue	(items,key,xr_new<FloatValue>(val,mn,mx,inc,decim),PROP_NUMERIC);	}
BOOLValue* 	  	__stdcall CPropHelper::CreateBOOL		(PropItemVec& items, shared_str key, BOOL* val)
{   return		(BOOLValue*)	AppendValue	(items,key,xr_new<BOOLValue>(val),PROP_BOOLEAN);        			}
VectorValue*   	__stdcall CPropHelper::CreateVector	(PropItemVec& items, shared_str key, Fvector* val, float mn, float mx, float inc, int decim)
{   return		(VectorValue*)	AppendValue	(items,key,xr_new<VectorValue>(val,mn,mx,inc,decim),PROP_VECTOR);	}
Flag8Value*		__stdcall CPropHelper::CreateFlag8   	(PropItemVec& items, shared_str key, Flags8* val, u8 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{   return		(Flag8Value*)	AppendValue	(items,key,xr_new<Flag8Value>(val,mask,c0,c1,flags),PROP_FLAG);    	}
Flag16Value*	__stdcall CPropHelper::CreateFlag16	(PropItemVec& items, shared_str key, Flags16* val, u16 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{   return		(Flag16Value*)	AppendValue	(items,key,xr_new<Flag16Value>(val,mask,c0,c1,flags),PROP_FLAG);    }
Flag32Value*	__stdcall CPropHelper::CreateFlag32	(PropItemVec& items, shared_str key, Flags32* val, u32 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{   return		(Flag32Value*)	AppendValue	(items,key,xr_new<Flag32Value>(val,mask,c0,c1,flags),PROP_FLAG);    }
Token8Value*	__stdcall CPropHelper::CreateToken8	(PropItemVec& items, shared_str key, u8* val, xr_token* token)
{   return		(Token8Value*)	AppendValue	(items,key,xr_new<Token8Value> (val,token),PROP_TOKEN);				}
Token16Value*	__stdcall CPropHelper::CreateToken16	(PropItemVec& items, shared_str key, u16* val, xr_token* token)
{   return		(Token16Value*)	AppendValue	(items,key,xr_new<Token16Value>(val,token),PROP_TOKEN);				}
Token32Value*	__stdcall CPropHelper::CreateToken32	(PropItemVec& items, shared_str key, u32* val, xr_token* token)
{   return		(Token32Value*)	AppendValue	(items,key,xr_new<Token32Value>(val,token),PROP_TOKEN);				}
RToken8Value* 	__stdcall CPropHelper::CreateRToken8	(PropItemVec& items, shared_str key, u8* val, xr_rtoken* token, u32 t_cnt)
{   return		(RToken8Value*)	AppendValue	(items,key,xr_new<RToken8Value>(val,token,t_cnt),PROP_RTOKEN);		}
RToken16Value* 	__stdcall CPropHelper::CreateRToken16	(PropItemVec& items, shared_str key, u16* val, xr_rtoken* token, u32 t_cnt)
{   return		(RToken16Value*)AppendValue	(items,key,xr_new<RToken16Value>(val,token,t_cnt),PROP_RTOKEN);		}
RToken32Value* 	__stdcall CPropHelper::CreateRToken32	(PropItemVec& items, shared_str key, u32* val, xr_rtoken* token, u32 t_cnt)
{   return		(RToken32Value*)AppendValue	(items,key,xr_new<RToken32Value>(val,token,t_cnt),PROP_RTOKEN);		}
TokenValueSH*   __stdcall CPropHelper::CreateTokenSH	(PropItemVec& items, shared_str key, u32* val, const TokenValueSH::Item* lst, u32 cnt)
{   return		(TokenValueSH*)	AppendValue	(items,key,xr_new<TokenValueSH>(val,lst,cnt),PROP_SH_TOKEN);		}
RListValue* 	 __stdcall CPropHelper::CreateRList	(PropItemVec& items, shared_str key, shared_str* val, shared_str* lst, u32 cnt)
{   return		(RListValue*)	AppendValue	(items,key,xr_new<RListValue>(val,lst,cnt),PROP_RLIST);       		}
U32Value*  		__stdcall CPropHelper::CreateColor   	(PropItemVec& items, shared_str key, u32* val)
{   return		(U32Value*)		AppendValue	(items,key,xr_new<U32Value>(val,0x00000000,0xffffffff,1,0),PROP_COLOR);}
ColorValue*		__stdcall CPropHelper::CreateFColor	(PropItemVec& items, shared_str key, Fcolor* val)
{   return		(ColorValue*)	AppendValue	(items,key,xr_new<ColorValue>(val),PROP_FCOLOR);        			}
VectorValue*	__stdcall CPropHelper::CreateVColor	(PropItemVec& items, shared_str key, Fvector* val)
{   return		(VectorValue*)	AppendValue	(items,key,xr_new<VectorValue>(val,0.f,1.f,0,0),PROP_VCOLOR);  		}
RTextValue* 	__stdcall CPropHelper::CreateRText	(PropItemVec& items, shared_str key, shared_str* val)
{   return		(RTextValue*)	AppendValue	(items,key,xr_new<RTextValue>(val),PROP_RTEXT);        				}
STextValue* 	__stdcall CPropHelper::CreateSText	(PropItemVec& items, shared_str key, xr_string* val)
{   return		(STextValue*)	AppendValue	(items,key,xr_new<STextValue>(val),PROP_STEXT);        				}
WaveValue* 		__stdcall CPropHelper::CreateWave		(PropItemVec& items, shared_str key, WaveForm* val)
{	return		(WaveValue*)	AppendValue	(items,key,xr_new<WaveValue>(val),PROP_WAVE);           			}
FloatValue* 	__stdcall CPropHelper::CreateTime		(PropItemVec& items, shared_str key, float* val, float mn, float mx)
{	return		(FloatValue*)	AppendValue	(items,key,xr_new<FloatValue>(val,mn,mx,0,0),PROP_TIME);    		}
ShortcutValue*	__stdcall CPropHelper::CreateShortcut	(PropItemVec& items, shared_str key, xr_shortcut* val)
{	return		(ShortcutValue*)AppendValue	(items,key,xr_new<ShortcutValue>(val),PROP_SHORTCUT);								}
//---------------------------------------------------------------------------
ChooseValue*	__stdcall CPropHelper::CreateChoose	(PropItemVec& items, shared_str key, shared_str* val, u32 mode, LPCSTR path, void* fill_param, u32 sub_item_count, u32 choose_flags)
{	
...
// obsolette
//---------------------------------------------------------------------------
CTextValue* 	__stdcall CPropHelper::CreateCText	(PropItemVec& items, shared_str key, LPSTR val, u32 sz)
{   return		(CTextValue*)	AppendValue	(items,key,xr_new<CTextValue>(val,sz),PROP_CTEXT);        				}
CTextValue* 	__stdcall CPropHelper::CreateTexture	(PropItemVec& items, shared_str key, LPSTR val, u32 sz)
{   return		(CTextValue*)	AppendValue	(items,key,xr_new<CTextValue>(val,sz),PROP_TEXTURE2);        				}
CListValue*    __stdcall 	CPropHelper::CreateCList	(PropItemVec& items, shared_str key, LPSTR val, u32 sz, xr_string* lst, u32 cnt)
{   return		(CListValue*)	AppendValue	(items,key,xr_new<CListValue>(val,sz,lst,cnt),PROP_CLIST);       			}
CTextValue* 	__stdcall CPropHelper::CreateCName	(PropItemVec& items, shared_str key, LPSTR val, u32 sz, ListItem* owner)
...
FloatValue* 	__stdcall CPropHelper::CreateAngle 	(PropItemVec& items, shared_str key, float* val, float mn, float mx, float inc, int decim)
...
VectorValue* 	__stdcall CPropHelper::CreateAngle3	(PropItemVec& items, shared_str key, Fvector* val, float mn, float mx, float inc, int decim)
...
RTextValue* 	__stdcall CPropHelper::CreateName		(PropItemVec& items, shared_str key, shared_str* val, ListItem* owner)
...
RTextValue* 	__stdcall CPropHelper::CreateNameCB	(PropItemVec& items, shared_str key, shared_str* val, TOnDrawTextEvent draw, RTextValue::TOnBeforeEditEvent before, RTextValue::TOnAfterEditEvent after)
{   RTextValue* V					= (RTextValue*) CreateRText	(items,key,val);
...
void __stdcall CPropHelper::FvectorRDOnBeforeEdit(PropValue* sender, Fvector& edit_val)
{
...
void __stdcall CPropHelper::FvectorRDOnDraw(PropValue* sender, xr_string& draw_val)
{
...
bool __stdcall CPropHelper::FvectorRDOnAfterEdit(PropValue* sender, Fvector& edit_val)
{
...
void __stdcall CPropHelper::floatRDOnBeforeEdit(PropValue* sender, float& edit_val)
{
...
void __stdcall CPropHelper::floatRDOnDraw(PropValue* sender, xr_string& draw_val)
{
	FloatValue* V	= dynamic_cast<FloatValue*>(sender); VERIFY(V);
    float val;    	val = rad2deg(*V->value);
    draw_sprintf	(draw_val,val,V->dec);
}
bool __stdcall CPropHelper::floatRDOnAfterEdit(PropValue* sender, float& edit_val)
{
...
void __stdcall CPropHelper::NameBeforeEdit(PropValue* sender, shared_str& edit_val)
{
	int cnt			=_GetItemCount(edit_val.c_str(),'\\');
	edit_val		= _SetPos(edit_val.c_str(),cnt-1,'\\');
}
void __stdcall CPropHelper::NameDraw(PropValue* sender, xr_string& draw_val)
{
...
bool __stdcall CPropHelper::NameAfterEdit(PropValue* sender, shared_str& edit_val)
{
	RTextValue* V	= dynamic_cast<RTextValue*>(sender); VERIFY(V);
    ListItem* L		= (ListItem*)sender->tag;
	return 			LHelper().NameAfterEdit(L,V->GetValue().c_str(),edit_val);
}
void __stdcall CPropHelper::CNameDraw(PropValue* sender, xr_string& draw_val)
{
	CTextValue* V	= dynamic_cast<CTextValue*>(sender); VERIFY(V);
	int cnt			=_GetItemCount(V->value,'\\');
	draw_val 		= _SetPos(V->value,cnt-1,'\\');
}
void __stdcall CPropHelper::CNameBeforeEdit(PropValue* sender, xr_string& edit_val)
{
	int cnt			=_GetItemCount(edit_val.c_str(),'\\');
	edit_val		= _SetPos(edit_val.c_str(),cnt-1,'\\');
}
bool __stdcall CPropHelper::CNameAfterEdit(PropValue* sender, xr_string& edit_val)
{
	CTextValue* V	= dynamic_cast<CTextValue*>(sender); VERIFY(V);
    ListItem* L		= (ListItem*)sender->tag;
