...
void CEditableObject::FillSurfaceProps(CSurface* SURF, LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateCaption		(items, PrepareKey(pref,"Face Count"),	shared_str().sprintf("%d",GetSurfFaceCount(SURF->_Name())));   
}
//---------------------------------------------------------------------------
...
void CEditableObject::FillBasicProps(LPCSTR pref, PropItemVec& items)
{
    ...
    V=PHelper().CreateCaption  	(items, PrepareKey(pref,"Transform\\BBox Min"),	shared_str().sprintf("{%3.2f, %3.2f, %3.2f}",VPUSH(GetBox().min)));
    V=PHelper().CreateCaption  	(items, PrepareKey(pref,"Transform\\BBox Max"),	shared_str().sprintf("{%3.2f, %3.2f, %3.2f}",VPUSH(GetBox().max)));
