...
#define TEMPLATE_SPECIALIZATION \
    template <typename _dist_type, typename _priority_queue, typename _vertex_manager, typename _vertex_allocator, bool euclidian_heuristics, typename _data_storage_base, \
              template <typename _T> class _vertex, template <typename _1, typename _2> class _builder_allocator_constructor, \
...
#define CSDijkstra \
    CDijkstra<_dist_type, _priority_queue, _vertex_manager, _vertex_allocator, euclidian_heuristics, _data_storage_base, _vertex, _builder_allocator_constructor, \
...
TEMPLATE_SPECIALIZATION
CSDijkstra::~CDijkstra() { xr_delete(m_data_storage); }

TEMPLATE_SPECIALIZATION
IC typename CSDijkstra::CDataStorage& CSDijkstra::data_storage() { return (*m_data_storage); }

TEMPLATE_SPECIALIZATION
IC const typename CSDijkstra::CDataStorage& CSDijkstra::data_storage() const { return (*m_data_storage); }
