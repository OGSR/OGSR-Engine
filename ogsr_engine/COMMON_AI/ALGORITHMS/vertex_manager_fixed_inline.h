...
TEMPLATE_SPECIALIZATION
IC bool CFixedVertexManager::is_closed(const CGraphVertex& vertex) const { return (is_visited(vertex) && !is_opened(vertex)); }
...
TEMPLATE_SPECIALIZATION
IC typename CFixedVertexManager::_path_id_type CFixedVertexManager::current_path_id() const { return (m_current_path_id); }
