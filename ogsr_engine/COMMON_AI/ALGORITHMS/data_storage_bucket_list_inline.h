...
TEMPLATE_SPECIALIZATION
 CBucketList::CDataStorage(const u32 vertex_count) : TManagerDataStorage(vertex_count)
{
...
TEMPLATE_SPECIALIZATION
 void CBucketList::add_best_closed()
{
...
TEMPLATE_SPECIALIZATION
 bool CBucketList::is_opened_empty()
{
...
TEMPLATE_SPECIALIZATION
 u32 CBucketList::compute_bucket_id(typename TManagerDataStorage::Vertex& vertex) const
{
    ...
    if (dist <= m_min_bucket_value)
        return 0;
    return static_cast<u32>(BucketCount * (dist - m_min_bucket_value) / (m_max_bucket_value - m_min_bucket_value));
}

TEMPLATE_SPECIALIZATION
 void CBucketList::verify_buckets() const
{
#if 0
...
TEMPLATE_SPECIALIZATION
 void CBucketList::add_to_bucket(typename TManagerDataStorage::Vertex& vertex, u32 m_bucket_id)
{
...
TEMPLATE_SPECIALIZATION
 void CBucketList::add_opened(typename TManagerDataStorage::Vertex& vertex)
{
...
TEMPLATE_SPECIALIZATION
 void CBucketList::decrease_opened(typename TManagerDataStorage::Vertex& vertex, const typename TManagerDataStorage::Vertex::Distance /*value*/)
{
    VERIFY(!is_opened_empty());
...
TEMPLATE_SPECIALIZATION
 void CBucketList::remove_best_opened()
{
    VERIFY(!is_opened_empty());
    verify_buckets();
...
TEMPLATE_SPECIALIZATION
typename TManagerDataStorage::Vertex& CBucketList::get_best()
{
    VERIFY(!is_opened_empty());
...
TEMPLATE_SPECIALIZATION
 void CBucketList::set_min_bucket_value(const typename TManagerDataStorage::Vertex::Distance min_bucket_value) { m_min_bucket_value = min_bucket_value; }

TEMPLATE_SPECIALIZATION
 void CBucketList::set_max_bucket_value(const typename TManagerDataStorage::Vertex::Distance max_bucket_value) { m_max_bucket_value = max_bucket_value; }
