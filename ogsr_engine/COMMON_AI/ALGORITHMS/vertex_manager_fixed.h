...
struct CVertexManagerFixed
{
    ...
    template <template <typename _T> class _vertex = CEmptyClassTemplate, template <typename _T1, typename _T2> class _index_vertex = CEmptyClassTemplate2,
              typename _data_storage = CBuilderAllocatorConstructor>
    class CDataStorage : public _data_storage::template CDataStorage<typename VertexManager<_vertex>::__vertex>
    {
    public:
        typedef typename _data_storage::template CDataStorage<typename VertexManager<_vertex>::__vertex> inherited;
