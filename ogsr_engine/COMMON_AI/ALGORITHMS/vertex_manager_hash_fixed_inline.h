...
#define TEMPLATE_SPECIALIZATION \
    ...
    template <template <typename _T> class _vertex, template <typename _T1, typename _T2> class _index_vertex, typename _data_storage>
...
TEMPLATE_SPECIALIZATION
IC typename CHashFixedVertexManager::_path_id_type CHashFixedVertexManager::current_path_id() const { return (m_current_path_id); }

TEMPLATE_SPECIALIZATION
IC bool CHashFixedVertexManager::is_opened(const CGraphVertex& vertex) const { return (vertex.opened()); }

TEMPLATE_SPECIALIZATION
IC u32 CHashFixedVertexManager::hash_index(const _index_type& vertex_id) const { return (u32(vertex_id) % hash_size); }
...
IC typename CHashFixedVertexManager::CGraphVertex& CHashFixedVertexManager::create_vertex(CGraphVertex& vertex, const _index_type& vertex_id)
{
    ...
    u32 index = hash_index(vertex_id);
    CGraphIndexVertex* __vertex = m_hash[index];
    if (!__vertex || (__vertex->m_path_id != current_path_id()) || (__vertex->m_hash != index))
        __vertex = 0;

    m_hash[index] = index_vertex;
    index_vertex->m_next = __vertex;
    index_vertex->m_prev = 0;
    if (__vertex)
        __vertex->m_prev = index_vertex;
