...
#define TEMPLATE_SPECIALIZATION \
    template <typename T1, typename T2, typename T3, typename T4, typename T5, bool T6, typename T7, typename T8, typename _DataStorage, typename _Parameters, \
              typename _dist_type, typename _index_type, typename _iteration_type>
...
TEMPLATE_SPECIALIZATION
IC bool CSolverPathManager::is_goal_reached(const _index_type& vertex_id) const { return (graph->is_goal_reached(vertex_id)); }

TEMPLATE_SPECIALIZATION
IC const _index_type& CSolverPathManager::get_value(const_iterator& i, bool reverse_search) const { return (graph->value(*best_node_index, i, reverse_search)); }
