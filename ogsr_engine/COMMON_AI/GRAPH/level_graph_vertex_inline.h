...
IC void CLevelGraph::project_point(const Fplane& plane, Fvector& point) const { point.y -= plane.classify(point) / plane.n.y; }

IC float CLevelGraph::distance(u32 vertex_id0, u32 vertex_id1) const { return (distance(vertex(vertex_id0), vertex(vertex_id1))); }

IC float CLevelGraph::distance(const CLevelGraph::CVertex* _node, u32 vertex_id1) const { return (distance(_node, vertex(vertex_id1))); }

IC float CLevelGraph::distance(u32 vertex_id0, const CLevelGraph::CVertex* _node) const { return (distance(vertex(vertex_id0), _node)); }

IC float CLevelGraph::distance(const CLevelGraph::CVertex* node0, const CLevelGraph::CVertex* node1) const { return (vertex_position(node0).distance_to(vertex_position(node1))); }

IC float CLevelGraph::distance(const Fvector& position, const u32 vertex_id) const { return (distance(position, vertex(vertex_id))); }

IC float CLevelGraph::distance(const u32 vertex_id, const Fvector& position) const { return (distance(position, vertex_id)); }

IC float CLevelGraph::distance(const u32 vertex_id, const CPosition& position) const { return (distance(vertex_position(position), vertex_id)); }

IC float CLevelGraph::distance(const CPosition& position, const u32 vertex_id) const { return (distance(vertex_position(position), vertex_id)); }
...
IC bool CLevelGraph::similar(const Fvector& tPoint0, const Fvector& tPoint1) const { return ((_abs(tPoint0.x - tPoint1.x) < EPS_L) && (_abs(tPoint0.z - tPoint1.z) < EPS_L)); }
...
IC void CLevelGraph::contour(CLevelGraph::SContour& _contour, u32 vertex_id) const { contour(_contour, vertex(vertex_id)); }
...
{
    return (compute_square(fAngle, fAngleOfView, NORMALIZE_NODE_COVER_HIGH(vertex, 0), NORMALIZE_NODE_COVER_HIGH(vertex, 1), NORMALIZE_NODE_COVER_HIGH(vertex, 2),
...
{
    return (compute_square(fAngle, fAngleOfView, NORMALIZE_NODE_COVER_LOW(vertex, 0), NORMALIZE_NODE_COVER_LOW(vertex, 1), NORMALIZE_NODE_COVER_LOW(vertex, 2),
                           NORMALIZE_NODE_COVER_LOW(vertex, 3)));
}

IC float CLevelGraph::compute_high_square(float fAngle, float fAngleOfView, u32 dwNodeID) const { return (compute_high_square(fAngle, fAngleOfView, vertex(dwNodeID))); }

IC float CLevelGraph::compute_low_square(float fAngle, float fAngleOfView, u32 dwNodeID) const { return (compute_low_square(fAngle, fAngleOfView, vertex(dwNodeID))); }
...
{
    return (cover_in_direction(angle, NORMALIZE_NODE_COVER_HIGH(vertex, 0), NORMALIZE_NODE_COVER_HIGH(vertex, 1), NORMALIZE_NODE_COVER_HIGH(vertex, 2),
                               NORMALIZE_NODE_COVER_HIGH(vertex, 3)));
}

IC float CLevelGraph::low_cover_in_direction(float angle, const CLevelGraph::CVertex* vertex) const
{
    return (cover_in_direction(angle, NORMALIZE_NODE_COVER_LOW(vertex, 0), NORMALIZE_NODE_COVER_LOW(vertex, 1), NORMALIZE_NODE_COVER_LOW(vertex, 2),
                               NORMALIZE_NODE_COVER_LOW(vertex, 3)));
}
...
IC float CLevelGraph::high_cover_in_direction(float angle, u32 vertex_id) const { return (high_cover_in_direction(angle, vertex(vertex_id))); }

IC float CLevelGraph::low_cover_in_direction(float angle, u32 vertex_id) const { return (low_cover_in_direction(angle, vertex(vertex_id))); }
