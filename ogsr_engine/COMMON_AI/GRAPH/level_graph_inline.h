...
#else
#if _M_X64
    return (valid_vertex_id(vertex_id)) ? (m_nodes->begin() + vertex_id) : (m_nodes->end() - 1);
#else
    return (m_nodes->begin() + vertex_id);
#endif
#endif
...
IC const Fvector CLevelGraph::vertex_position(const CLevelGraph::CVertex* vertex) const { return (vertex_position(*vertex)); }
...
IC bool CLevelGraph::inside(const CVertex* vertex, const CLevelGraph::CPosition& vertex_position) const { return (inside(*vertex, vertex_position)); }

IC bool CLevelGraph::inside(const CVertex* vertex, const Fvector& position) const { return (inside(*vertex, position)); }

IC bool CLevelGraph::inside(const u32 vertex_id, const CLevelGraph::CPosition& vertex_position) const { return (inside(vertex(vertex_id), vertex_position)); }
...
IC bool CLevelGraph::inside(const u32 vertex_id, const Fvector& position, const float epsilon) const { return (inside(vertex(vertex_id), position, epsilon)); }

IC bool CLevelGraph::inside(const u32 vertex_id, const Fvector2& position) const
{
    const int pxz = iFloor(((position.x - header().box().min.x) / header().cell_size() + .5f)) * m_row_length + iFloor((position.y - header().box().min.z) / header().cell_size() + .5f);
    VERIFY(pxz < NodePosition::MAX_XZ);
    const bool b = vertex(vertex_id)->position().xz() == u32(pxz);
    return (b);
}
...
IC float CLevelGraph::vertex_plane_y(const CLevelGraph::CVertex* vertex, const float X, const float Z) const { return (vertex_plane_y(*vertex, X, Z)); }

IC float CLevelGraph::vertex_plane_y(const CLevelGraph::CVertex* vertex) const { return (vertex_plane_y(*vertex)); }

IC float CLevelGraph::vertex_plane_y(const u32 vertex_id, const float X, const float Z) const { return (vertex_plane_y(vertex(vertex_id), X, Z)); }
...
ICF bool CLevelGraph::CVertex::operator<(const CLevelGraph::CVertex& vertex) const { return (position().xz() < vertex.position().xz()); }
...
IC bool CLevelGraph::is_accessible(const u32 vertex_id) const { return (valid_vertex_id(vertex_id) && m_access_mask[vertex_id]); }
...
{
    if ((position.x < header().box().min.x - header().cell_size() * .5f) || (position.x > header().box().max.x + header().cell_size() * .5f) ||
        (position.z < header().box().min.z - header().cell_size() * .5f) || (position.z > header().box().max.z + header().cell_size() * .5f))
        return (false);
...
IC bool operator<(const CLevelGraph::CVertex& vertex, const u32& vertex_xz) { return (vertex.position().xz() < vertex_xz); }

IC bool operator>(const CLevelGraph::CVertex& vertex, const u32& vertex_xz) { return (vertex.position().xz() > vertex_xz); }

IC bool operator==(const CLevelGraph::CVertex& vertex, const u32& vertex_xz) { return (vertex.position().xz() == vertex_xz); }

IC bool operator<(const u32& vertex_xz, const CLevelGraph::CVertex& vertex) { return (vertex_xz < vertex.position().xz()); }
