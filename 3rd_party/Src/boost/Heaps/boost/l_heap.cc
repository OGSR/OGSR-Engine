...
// Version: $Id: l_heap.cc,v 1.5 1999/07/13 01:42:09 kuehl Exp $ 
...
template <typename T>
inline void
typename lazy_fibonacci_heap_base<T>::node::cut(node* child)
{
...
inline void
typename lazy_fibonacci_heap<T, Comp>::add_root(node* n) const
{
...
template <typename T, typename Comp>
inline void
typename lazy_fibonacci_heap<T, Comp>::find_min() const
{
  if (m_size == 0)
    m_min = 0;
  else
...
template <typename T, typename Comp>
inline void
typename lazy_fibonacci_heap<T, Comp>::cut(node* n)
{
  node* p = n->parent();
  p->cut(n);
  switch (p->state())
...
template <typename K>
inline void
typename lazy_fibonacci_heap<T, Comp>::decrease(pointer n, K const& data)
{
  n->data(data);
...
template <typename K>
inline void
typename lazy_fibonacci_heap<T, Comp>::increase(pointer n, K const& data)
{
  n->data(data);
...
template <typename K>
inline void
typename lazy_fibonacci_heap<T, Comp>::change(pointer n, K const& data)
{
  T comp(n->data());
  comp = data;
...
template <typename K>
inline void
typename lazy_fibonacci_heap<T, Comp>::change_top(K const& data)
{
  if (m_min == 0)
    find_min();
  change(m_min, data);
...
template <typename T, typename Comp>
inline void
typename lazy_fibonacci_heap<T, Comp>::remove(pointer n)
{
  switch (n->state())
    {
    case cut_self:
...
template <typename T, typename Comp>
inline void
typename lazy_fibonacci_heap<T, Comp>::pop()
{
  if (m_min == 0)
    find_min();
